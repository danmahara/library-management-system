...
require_once ('../librarian/LibrarianManager.php');
session_start();
...
if (!empty($_GET['book_id'])) {
    ...
    if (!empty($_POST)) {
        ...
        $response = $bookManager->updateBook($bookTitle, $author, $isbn, $avialable_copies);
        $responseDecode = json_decode($response, true);

    }
    $book = $bookManager->getBookDetails($id);
    $bookDecode = json_decode($book, true);
    // print_r($bookDecode);
...
<body>
    <div class="container">
        <div class="wrap">
            <h2>Edit Book</h2>
            <h2><a href="../librarian/manageBooks.php" class="viewBooks" style="">View Books </a> </h2>
        ...
        <?php } else { ?>
            <p class="error"><?= isset($responseDecode['message']) ?></p>

        <?php } ?>

        <form id="addBookForm" action="" method="post">
            <?php foreach ($bookDecode as $key => $book) { ?>
                <div>
                ...
                <div>
                    ...
                    <input type="text" id="isbn" value="<?= $book['isbn']; ?>" name="isbn" required>
                ...
                <div>
                    ...
                    <input type="number" id="availableCopies" name="availableCopies"
                        value="<?= isset($book['available_copies']) ? htmlspecialchars($book['available_copies']) : ''; ?>"
                        required>
                    <span class="error" id="availableCopiesError"></span>

                </div>
                <div>
                    <button type="button" onclick="validateForm()">Update Book</button>
                </div>
            <?php } ?>
        </form>
    </div>
