...
<head>
    ...
    <script>
        function checkAvailability(copies) {
            if (copies == 0) {
                alert('Not in library');
                return false;
            }
            return true;
        }
    ...
    <div class="container">
        ...
        <?php if (!empty($_SESSION['success_message']) && isset($_SESSION['success_message'])) {
            if (isset($_SESSION['success_message'])) { ?>
                ...
                ?></p>
            <?php } else ?>
            <p class="success"><?= isset($_SESSION['error_message']);
                        ...
                        <?php if (!empty($responseDecode) && isset($responseDecode['success']) && $responseDecode['success'] == true && !empty($responseDecode['data'])) {
                            foreach ($responseDecode['data'] as $key => $book) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="makeBorrow.php?book_id=<?= $book['id']; ?>"
                                            onclick="return checkAvailability(<?= $book['available_copies']; ?>);">Borrow</a>
                                    </td>
                                </tr>
                            <?php }
                        } ?>
